...
bool mqttReconnect() {
  ...
  String clientId = String("D1DOORBELL") + "-";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (s == "open") {
    DEBUG_PRINTLN("MQTT Requests open door");
...
void setup() {
  ...
  pinMode(RINGING_INDICATOR_PIN, OUTPUT);
...
void loop() {
  ...
  if (lastButtonpressStart != 0) {
    if (lastButtonpressStart + OPEN_DURATION < millis()) {
      DEBUG_PRINTLN("Timeout reached for pressing the button");
      digitalWrite(OPEN_PIN, LOW);
      lastButtonpressStart = 0;
    }
  }

  if (ringingIndicatorEnds != 0) {
    if (millis() > ringingIndicatorEnds) {
      ringingIndicatorEnds = 0;
